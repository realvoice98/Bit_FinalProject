...
function GosuInfo() {
    ...
    console.log(`gosuDetails`, gosuDetails.id);
    return (
        <GosuDetailContainer>
            ...
            <ReviewMoreBtn  onClick={()=>{
                ...
                fetch(`/chat/new`, {
                    ...
                    .then((res)=>{
                        console.log(res)
                        localStorage.setItem('surveyId',gosuDetails.id)
                        console.log('보내기 성공 ')
                        let number = res.room;
                        localStorage.setItem('room',number);
                        console.log(gosuDetails)
                        fetch(`/matchedstart`, {
                            method: 'PUT',
                            headers: {
                                'Content-Type': 'application/json',
                            ...
                            body: JSON.stringify(
                                {userEmail: localStorage.getItem('email'),gosuEmail: localStorage.getItem('email2'),surveyId: gosuDetails.surveyId}
                            )
                        })
                            .then(res => res.json())
                            .then((res)=>{
                                console.log('status변경 완료')
                            })

                        window.location.href='/chat/sender/room'
                        // setRoom(room)
                    });
            }}>상담하기</ReviewMoreBtn>
